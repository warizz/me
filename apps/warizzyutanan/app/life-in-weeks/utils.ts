import { differenceInWeeks, addWeeks, startOfWeek, isSameWeek } from "date-fns";
...
export const getColorForEvent = (event: LifeEvent) => {
  ...
  let attempts = 0;
  while (existingHues.some(h => Math.abs(h - finalHue) < 20 || Math.abs(h - finalHue) > 340) && attempts < 18) {
